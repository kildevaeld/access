...
class SQLStore {
    ...
    addRule(role, rule) {
        ...
        var self = this;
        return this._coTransaction(knex, function* (knex, thx) {
            role = yield knex(self._tables.roles).transacting(thx).where('name', role.name).first('id');
            if (!role) {
                throw new Error('role \'' + role.name + '\' not found');
            }
            var perm = yield knex(table).transacting(thx).where(_.omit(rule, 'predicate')).first('id');
            if (!perm) {
            ...
            return yield knex(self._tables.joint).transacting(thx).insert({
    ...
    _coTransaction(knex, fn) {
        var self = this;
        return knex.transaction(function (thx) {
            return (0, _co2['default'])(function* () {
